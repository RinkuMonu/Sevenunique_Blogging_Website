...
export default function LifeFoodSection({ tabs, items }) {
  // keep track of which tab is active (default first tab)
  const [activeTab, setActiveTab] = useState(tabs[1]);

  // filter items based on active tab
  const filteredItems = items.filter(
    (item) => item.category === activeTab || activeTab === "healthy"
  );
    ...
    <section className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center gap-6 mb-6">
         <h2 className="text-2xl md:text-3xl underline font-bold text-red-600 flex items-center">
          Food <span><ChevronRight className="ml-1 w-5 h-5" /></span>
        </h2>
