...
export default function LifeRelationshipSection({ tabs, items }) {
  // Active tab state
  const [activeTab, setActiveTab] = useState(tabs[0]);

  // Filter items by category, show all when "Trending"
    ...
    <section className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center gap-6 mb-6">
        <h2 className="text-2xl md:text-3xl underline font-bold text-red-600 flex items-center">
          Relationship <span><ChevronRight className="ml-1 w-5 h-5" /></span>
        </h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        {filteredItems.length > 0 ? (
          ...
          <p className="col-span-4 text-center text-gray-500">
            No relationship articles available
          </p>
        )}
      </div>
