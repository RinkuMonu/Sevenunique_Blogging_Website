...
export default function LifeRecipeSection({ tabs, items }) {
  // Track active tab
  const [activeTab, setActiveTab] = useState(tabs[0]);

  // Filter items: show all if "Trending" else match category
    ...
    <section className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center gap-6 mb-6">
        <h2 className="text-2xl md:text-3xl underline font-bold text-red-600 flex items-center">
          Recipe <span><ChevronRight className="ml-1 w-5 h-5" /></span>
        </h2>
