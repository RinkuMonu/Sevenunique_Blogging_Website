...
const MovieReviewSlider = () => {
  ...
  const [itemsPerPage, setItemsPerPage] = useState(3); // default

  const filters = ['Trending', 'Hindi', 'English', 'Tamil', 'Telugu'];

  // 🔥 Handle responsiveness for itemsPerPage
  useEffect(() => {
  ...
  const handleNext = () => {
    ...
    setCurrentIndex(prev => (prev >= maxIndex ? 0 : prev + 1)); // 🔁 loop
  };
              ...
              >
                {filteredItems.map(item => (
                  <div key={item.id} className="w-full flex-shrink-0" style={{ flexBasis: `${100 / itemsPerPage}%` }}>
                    <div className=" rounded-lg overflow-hidden flex flex-col  h-full md:mx-2">
                      <Image
                        src={`/${item.imageUrl}`}   // ✅ maps to /public/sports/img7.png
                        alt={item.title}
                        width={500}
                      ...
                      <div className="p-4 flex flex-col">
                        ...
                        <div className="flex justify-end">
                          <button className="mt-4 bg-red-600 text-white text-sm font-bold py-1 px-6 rounded-full hover:bg-red-700 transition-colors duration-300 ">
                            View more
                          </button>
                        </div>
                      </div>
